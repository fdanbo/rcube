...
def rotate(mat, elts):
    previous = elts[0]
    for e in elts[1:]:
        # properly I think we should be swapping here, but since we know we're
        # starting with an identity matrix, using 0 and 1 directly makes the
        # code simpler.
...
def createRCubeMatrices():
    ...
    rot = numpy.identity(54, numpy.uint8)
    rotate(rot, [0, 2, 8, 6])
    rotate(rot, [1, 5, 7, 3])
